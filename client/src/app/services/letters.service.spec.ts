...
describe('LettersService', () => {
    ...
    it('changeLetterFromReserve', () => {
        const str = 'a';
        const easel = new EaselObject(true);
        easel.easelLetters = [A, C];
        spyOn<any>(easel, 'contains').and.returnValue(true);
        spyOn<any>(service['reserveService'], 'isReserveEmpty').and.returnValue(false);
        const spy = spyOn<any>(service['easelLogisticsService'], 'getLetterFromEasel');

        service.changeLetterFromReserve(str, easel);
        expect(spy).toHaveBeenCalled();
    });

    it('changeLetterFromReserve else', () => {
        const str = 'a';
        const easel = new EaselObject(true);
        easel.easelLetters = [A, C];
        ...
        expect(service.changeLetterFromReserve(str, easel)).toBeFalse();
    });

    it('placeLettersInScrabble', () => {
        const easel = new EaselObject(true);
        const user = true;
        const vec2: Vec2 = { x: 1, y: 1 };
        const command: ChatCommand = { word: 'sa', position: vec2, direction: 'h' };
        const spy = spyOn<any>(service, 'placeLettersWithDirection');
        service.placeLettersInScrable(command, easel, user);
        expect(spy).toHaveBeenCalled();
    });
    ...
    it('placeLettersInScrabble', () => {
        const easel = new EaselObject(true);
        const user = true;
        easel.indexOfEaselLetters = [0, 1, 2, 3, 4, 5, 6];
        const vec2: Vec2 = { x: 1, y: 1 };
        const command: ChatCommand = { word: 'sa', position: vec2, direction: 'h' };

        service.placeLettersInScrable(command, easel, user);
        expect(service.usedAllEaselLetters).toBe(true);
    });

    it('placeLettersWithDirection', () => {
        const vec2: Vec2 = { x: 1, y: 1 };
        const command: ChatCommand = { word: 'sa', position: vec2, direction: 'h' };
        const spy = spyOn<any>(service, 'placeLetter');
        service.placeLettersWithDirection(command);
        expect(spy).toHaveBeenCalled();
    });

    it('placeLettersWithDirection v', () => {
        const vec2: Vec2 = { x: 1, y: 1 };
    ...
    it('wordIsPlacable', () => {
        ...
        const easel = new EaselObject(true);
        const x = 2;
        service.wordIsPlacable(command, easel);
        expect(x).toBe(2);
    });
    ...
    // it('wordIsPlacable call charAt', () => {
    //     service.tiles[0][0] = C;
    //     const vec2: Vec2 = { x: 1, y: 1 };
    //     const command: ChatCommand = { word: 'a', position: vec2, direction: 'h' };
    //     const easel = new EaselObject(true);
    //     easel.easelLetters = [A, A, A, A, A, A, A];
    //     const v = service.wordIsPlacable(command, easel);
    //     expect(v).toBeTrue();
    ...
    it('wordIsPlacable contains', () => {
        const vec2: Vec2 = { x: 1, y: 1 };
        const command: ChatCommand = { word: 'sa', position: vec2, direction: 'h' };
        const easel = new EaselObject(true);
        spyOn<any>(easel, 'contains').and.returnValue(true);
        expect(service.wordIsPlacable(command, easel)).toBe(true);
    });
    ...
    //     const letterArray: Letter[] = [S, A, C, C, C, C, C, C, C, C, C, C, C, C, C];
    //     service.tiles[0] = letterArray;
    //     const spyOnWordInEasel = spyOn<any>(service, 'wordInEasel').and.callFake(() => {
    //         return true;
    //     });
    //     const wordIsplacable: boolean = service.wordIsPlacable(command);
    //     expect(spyOnWordInEasel).toHaveBeenCalled();
